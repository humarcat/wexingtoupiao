...
define('DISCUZ_ROOT', KJ_DIR_LIB.'/components');
//note 普通的 http 通知方式
if(!defined('IN_UC')) {
	...
	include KJ_DIR_DATA . '/config/cfg.uc.php';
	...
	$timestamp = time();
	$get_time = isset($get['time']) ? $get['time'] : 0;
	if($timestamp - $get_time > 3600) {
...
class uc_note {
	...
	function synlogin($get, $post) {
		...
		//同步登录
		$arr_fields = array();
		(!isset($uid)) ? $arr_fields['user_id'] = $uid : $arr_fields['user_name'] = $username;
		$arr = cls_obj::get("cls_user")->on_login($arr_fields , 1 , true);
	}
	...
	function synlogout($get, $post) {
		...
		//同步退出
		cls_obj::get("cls_user")->on_loginout(true);
	}
	...
	function updatepw($get, $post) {
		...
		//修改密码
		$arr = cls_obj::get("cls_user")->get_user($username , false);
		if(isset($arr[$username])) {
			$uid = $arr[$username];
			cls_obj::get("cls_user")->on_update_pwd('' , $uid , false);
		}
		return API_RETURN_SUCCEED;
	}
