...
class mod_sys_user_action extends inc_mod_admin {
	...
	function get_pagelist() {
		...
		$arr_search_key = array(
			...
			'score1' => fun_get::get("s_score1"),
			'score2' => fun_get::get("s_score2"),
			'experience1' => fun_get::get("s_experience1"),
			'experience2' => fun_get::get("s_experience2"),
			'user' => fun_get::get("s_user"),
			's_action' => fun_get::get("s_action"),
		);
		...
		if( !empty($arr_search_key['score1']) ) $arr_where_s[] = "action_score>='" . $arr_search_key['score1'] . "'";
		if( !empty($arr_search_key['score2']) ) $arr_where_s[] = "action_score<='" . $arr_search_key['score2'] . "'";
		if( !empty($arr_search_key['experience1']) ) $arr_where_s[] = "action_experience>='" . $arr_search_key['experience1'] . "'";
		if( !empty($arr_search_key['experience2']) ) $arr_where_s[] = "action_experience<='" . $arr_search_key['experience2'] . "'";
		if( !empty($arr_search_key['s_action']) ) $arr_where_s[] = "action_key='" . $arr_search_key['s_action'] . "'";
		if( !empty($arr_search_key['user']) ) {
			if(cls_config::USER_CENTER=='user.klkkdj') {
				...
				$ids = implode("," , $arr_ids);
				if(!empty($ids)) {
					$arr_where_s[] = "action_user_id in(" . $ids . ")";
				} else {
					$arr_where_s[] = "action_user_id=0";
				}
			} else {
				...
				if(!empty($ids)) {
					...
					$arr_where_s[] = "action_user_id='" . (int)$arr_search_key['user'] . "'";
				}
			}
		}
		//合并查询数组
	...
	function sql_list($str_where = "" , $lng_page = 1) {
		...
		$sort = str_replace("user_name" , "action_user_id" , $arr_config_info["sortby"]);
	...
	function config_save() {
		...
		foreach($arr_fields as $item => $list) {
			...
			foreach($list as $key => $val) {
				if(fun_is::set($key . "_" . $item)) {
					$arr_fields[$item][$key] = fun_get::get($key . "_" . $item);
				}
			}
		}
