...
class cls_db {
	...
	function close(){
		if(!isset($this->perms["db_connid"])) return false;
		$close = mysql_close($this->perms["db_connid"]);
		unset($this->perms["db_connid"]);
		return $close;
	}
	...
	function get_pageinfo($tab , $where = '' , $page = 1 , $pagesize = 10 , $arr_fields = array()) {
		$arr_fields[] = "count(1) as num";
		$fields = implode("," , $arr_fields);
		$obj_rs = $this->get_one("SELECT " . $fields . " FROM " . $tab . $where);
		if(!empty($obj_rs)) {
			$arr_return = $obj_rs ;
		} else {
			$arr_return['total'] = array() ;
		}
		$arr_return['total'] = $arr_return['num'];
		unset($arr_return['num']);
