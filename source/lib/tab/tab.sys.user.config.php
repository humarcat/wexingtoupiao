...
class tab_sys_user_config {
	 ...
	 */
	static function get_info($msg_key , $msg_type , $uid = 0 , $arr_default = array()) {
		$arr = self::get_config("config_info" , $uid);
		$arr_config_info = array();
		$pagesize = isset($arr_default['pagesize']) ? $arr_default['pagesize'] : 10;
		(isset($arr[$msg_type][$msg_key]["pagesize"])) ? $arr_return["pagesize"] = $arr[$msg_type][$msg_key]["pagesize"] : $arr_return["pagesize"] = cls_config::get("pagesize" , "sys" , $pagesize);
	...
	static function save_sort($msg_sortby , $msg_key , $msg_type , $uid = 0) {
		...
		$bysort = 'asc';
		$sortmode = cls_config::get("sortmode");
