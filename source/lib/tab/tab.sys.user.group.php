...
class tab_sys_user_group {
	...
	static function get_perms($key) {
		if( empty(self::$perms) ) {
			self::$perms = array(
				"limit_dir" => array(cls_language::get("admin") => "admin" , "店铺" => "shop"),
			);
		}
		$arr_return = array();
	...
	static function on_save($arr_fields , $where = '') {
		...
		if( empty($where) ) {
			...
			if(!isset($arr_fields['group_sort']) || empty($arr_fields['group_sort'])) {
				$obj_rs = $obj_db->get_one("select max(group_sort) as sort from " . cls_config::DB_PRE . "sys_user_group where group_pid=" . $arr_fields["group_pid"]);
				(!empty($obj_rs))? $arr_fields['group_sort'] = $obj_rs["sort"] + 1 : $arr_fields['group_sort'] = 1;
			}
			if($arr_fields["group_pid"] > 0) {
				$obj_rs = $obj_db->get_one("select group_pids from ".cls_config::DB_PRE."sys_user_group where group_id=".$arr_fields["group_pid"]);
			...
			//插入到用户表
			$arr = $obj_db->on_insert(cls_config::DB_PRE."sys_user_group",$arr_fields);
			if($arr['code'] == 0) {
				...
				if(empty($arr_return['id'])) {
					...
					$obj_rs = $obj_db->get_one("select group_id from ".cls_config::DB_PRE."sys_user_group where ".$where);
			...
			//修改数据表
			$arr = $obj_db->on_update(cls_config::DB_PRE."sys_user_group" , $arr_fields , $where);
			if($arr['code'] != 0) {
				$arr_return['code'] = $arr['code'];
				$arr_return['msg']  = cls_language::get("db_edit");
			}
	...
	static function on_delete($arr_id , $where = '') {
		...
		$arr_return=cls_obj::db_w()->on_delete(cls_config::DB_PRE."sys_user_group" , $where);
		return $arr_return;
	}
	/* 移动分组
	...
	static function on_move($id , $pid , $pids='') {
		$obj_db = cls_obj::db_w();
		if(empty($pids)) {
			$obj_rs = $obj_db->get_one("select group_pids from ".cls_config::DB_PRE."sys_user_group where group_id=".$pid);
			if(!empty($obj_rs) && !empty($obj_rs["group_pids"])) {
		...
		$arr_return = $obj_db->on_exe("update ".cls_config::DB_PRE."sys_user_group set group_pid=".$pid.",group_pids='".$pids."' where group_id='".$id."'");
		if($arr_return["code"] == 0) {
			//循环移动子集
			$obj_result = $obj_db->select("select group_id from " . cls_config::DB_PRE . "sys_user_group where group_pid='".$id."'");
	...
	static function get_list_layer($pid = 0 , $layer = 1 , $where = '') {
		...
		$obj_result = $obj_db->select("SELECT * FROM ".cls_config::DB_PRE."sys_user_group" . $str_where . " order by group_sort,group_id");
		while( $obj_rs = $obj_db->fetch_array($obj_result) ) {
			$obj_rs["layer"] = $layer;
