...
class tab_vote_act {
	...
	static function on_save($arr_fields , $where = '') {
		...
		if( empty($where) ) {
			...
			//必填项检查
			if(!isset($arr_fields['act_name']) || empty($arr_fields['act_name'])) {
				$arr_return['code'] = 113;
			...
			//插入到表
			$arr = $obj_db->on_insert(cls_config::DB_PRE."vote_act",$arr_fields);
			if($arr['code'] == 0) {
				$arr_return['id'] = $obj_db->insert_id();
				//其它非mysql数据库不支持insert_id 时
				if(empty($arr_return['id'])) {
					$where  = "act_name='" . $arr_fields['act_name'] . " and act_addtime='".$arr_fields['act_addtime'] . "'";
					$obj_rs = $obj_db->get_one("select act_id from ".cls_config::DB_PRE."vote_act where ".$where);
			...
			//修改数据表
			$arr = $obj_db->on_update(cls_config::DB_PRE."vote_act" , $arr_fields , $where);
			if($arr['code'] != 0) {
				$arr_return['code'] = $arr['code'];
				$arr_return['msg']  = cls_language::get("db_edit");
			}
	...
	static function on_delete($arr_id , $where = '') {
		...
		$arr_return=$obj_db->on_delete(cls_config::DB_PRE."vote_act" , $where);
		return $arr_return;
	}
}