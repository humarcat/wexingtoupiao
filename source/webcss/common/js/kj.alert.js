...
kj.alert = function( msg , fun , data) {
	var arr = kj.url_toarray(location.href);
	if(arr['url'].substr(-7) == 'app.php') {
		var html = '<div style="float:left;margin:10px;">'+msg+'</div>';
		kj.dialog({'html':html,'id':'alert','type':'html','title':'消息提示','max_w':350,'min_w':200,'showbtnmax':false,'showbtnhide':false});
	} else {
		alert(msg);
